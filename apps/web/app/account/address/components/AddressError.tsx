...
export const AddressErrorState = ({ refetch, isFetching }: AddressErrorStateProps) => {
    ...
    >
        ...
        <h3 className={`text-xl font-bold mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>Unable to load addresses</h3>
        <p className={`mb-6 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>We encountered an issue fetching your data.</p>
        <button
            onClick={() => refetch()}
            disabled={isFetching}
            className="inline-flex items-center gap-2 px-6 py-3 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-all active:scale-95 disabled:opacity-50 disabled:active:scale-100 cursor-pointer"
        >
