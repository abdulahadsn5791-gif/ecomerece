...
export class InventoryAggregate extends AggregateRoot {
    ...
    get lowStockThreshold() { return this._lowStockThreshold; }
    ...
    static create(data: createInventoryProps): InventoryAggregate {
        return new InventoryAggregate(data.id,
            data.variantId,
            data.available,
    ...
    static rehydrate(_id: Id,
        ...
        _version: Quantity,
        _createdAt: EffectiveDate): InventoryAggregate {
        return new InventoryAggregate(_id,
            _variantId,
            _available,
            _reserved,
    ...
    deleteInventory(reason: Reason, actorId: Id) {
        if (!this._available.isZero) throw new BadRequestError("First clear the available stocks  to continue");
        if (!this._reserved.isZero) throw new BadRequestError("First clear the reserve stocks to continue");
